"""
Docling image parser: OCR images using Docling DocumentConverter.
...
class DoclingImageParser(MediaParser):
    """
    MediaParser for images using Docling image pipeline.
    Uses image_parser_lang from config.
    """
    ...
    def document_parse(self, content: bytes, filename: str) -> NormalizedDoc:
        options = ImagePipelineOptions(engine="docling", lang=self._lang)
