"""
Pytesseract image parser: OCR images using Tesseract via pytesseract.
...
class PytesseractImageParser(MediaParser):
    """
    MediaParser for images (.png, .jpg, etc.) using pytesseract.
    Uses image_parser_lang from config (e.g. slv+eng).
    """
    ...
    def document_parse(self, content: bytes, filename: str) -> NormalizedDoc:
        options = ImagePipelineOptions(engine="pytesseract", lang=self._lang)
